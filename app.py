...
def index():
    ...
    for user in all_users:
        pts_gain[user] = max(0, pts_day(user, history_today) -
                             pts_day(user, history_yesterday))

    gains = sorted(pts_gain.items(), key=lambda item: item[1], reverse=True)
    gains = [{'UserId': gain[0], 'Score': gain[1]} for gain in gains]
